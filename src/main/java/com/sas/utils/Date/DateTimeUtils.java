...
public class DateTimeUtils {
	...
	public static String formatDate(Date date, Locale locale) {
		if (date == null && debug) {
			System.out.println(LOG_TAG + " formatDate >> Supplied date is null");
		}
		...
		if (debug) {
			System.out.println(LOG_TAG + " formatDate >> Formatting using " + iso8601Format.getTimeZone().getDisplayName() + " | " + iso8601Format.getTimeZone().getID());
		}
	...
	public static Date formatDate(String dateString, Locale locale) {
		...
		if (dateString != null) {
			try {
				...
				if (debug) {
					System.out.println(LOG_TAG + " formatDate >> Fail to parse supplied date string >> " + dateString);
					e.printStackTrace();
				}
			}
		}
	...
	public static String formatWithPattern(Date date, String pattern, Locale locale) {
		if (date == null && debug) {
			System.out.println(LOG_TAG + " FormatWithPattern >> Supplied date is null");
		}
	 ...
	 * 获取当前时间
	 *
	 * @param dateTimePattern3
	 * @return
	 */
	public static String getCurrentTimeStr(String dateTimePattern3) {
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern(dateTimePattern3);
	...
	public static Date getCurrentTime() {
		LocalDateTime date = LocalDateTime.now();
		return localDateTime2Date(date);
	}
